...
<body>
		...
		<div data-options="region:'north',title:''" style="height: 50px; background: #F4F4F4;">
			<form id="searchForm">
				<table>
					<tr>
						<th>企业名称：</th>
						<td><select id="corpId" name="corpId">
						...
						<td><select id="typeId" name="typeId">
								<option value="">----- 请选择-----</option>
								<c:forEach items="${typelist }" var="montype">
									<option value="${montype.id }">${montype.title }</option>
								</c:forEach>
						</select></td>
	...
	$(function() {
		$("#list_data").datagrid({
			...
			fit : true,//自动大小 
			loadMsg : '数据加载中请稍后……',
			url : basepath+'/monitorval/datagrid',
			remoteSort : false,
			fitColums : true,
			checkOnSelect : true,
			...
			columns : [ [ {
				...
				formatter:function(value,row){
			    	var str = "";
			    	if(value=="0"){
			    		str = "<font style='color:red'>超标</font>";
			    	}else if(value=="1"){
		...
		function addFun() {
			var monitval_add_dialog = $('#monitorval_window').dialog({
			title : '新增检测结果',
			width : 600,
			height : 400,
			closed : false,
			cache : false,
			href : basepath+'/monitorval/dataform',
			modal : true,
			buttons : [ {
				text : '添        加',
				handler : function() {
				$('#monitorvalForm').form('submit',
					{
						url : basepath+'/monitorval/create',
						success : function(data) {
							var json = $.parseJSON(data);
							if (json.success) {
								$('#list_data').datagrid('load');
								monitval_add_dialog.dialog('close');
							}
							$.messager.show({
								title : '提示',
								msg : result.msg,
		...
		function editFun() {
			...
			if (checkedRows.length == 1) {
				var val_window_dialog = $('#monitorval_window').dialog({
				title : '检测结果编辑',
				width : 600,
				height : 500,
				closed : false,
				cache : false,
				href : basepath+'/monitorval/dataform?id='+checkedRows[0].id,
				modal : true,
				buttons : [ {
					text : '保      存',
					handler : function() {
						$('#monitorvalForm').form('submit',{
							url : basepath+'/monitorval/update',
							success : function(data) {
								var result = $.parseJSON(data);
								if (result.success) {
									$('#list_data').datagrid('load');
									val_window_dialog.dialog('close');
								}
								$.messager.show({
									title : '提示',
									msg : result.msg,
